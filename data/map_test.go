...
var mapCases = map[string]interface{}{
	...
	// map filterf
	"data:map{x = 2, y = 3}.filterf()":           "filter: requires condition function at file:30",
	"data:map{x=2,y=3}.filterf(fun(i,v, v < 3))": "map[x:{2}]",
	"data:map{x=2}.filterf(1)":                   "filter: arg is not a function at file:22",
	"data:map{x=2}.filterf(fun(i,v,v))":          "filter: function returned non-boolean at file:27",
	"data:map{x=2}.filterf(fun(i,v,v ++ v))":     "missing term at file:33",

	// map filter
	"data:map{x = 2, y = 3}.filter()":      "filter: requires condition function at file:29",
	"data:map{x=2,y=3}.filter(value < 3)":  "map[x:{2}]",
	"data:map{x=2}.filter(value)":          "filter: function returned non-boolean at file:21",
	"data:map{x=2}.filter(value ++ value)": "missing term at file:28",

	// map mapf
	"data:map{x=2}.mapf()":               "map: requires 1 arg at file:18",
	"data:map{x=2}.mapf(fun(i,v,v+1))":   "map[x:{3}]",
	"data:map{x=2}.mapf(builtin:number)": "map[x:builtin:number: must have 1 arg]",

	// map map
	"data:map{x=2}.map()":        "map: requires 1 arg at file:17",
	"data:map{x=2}.map(value+1)": "map[x:{3}]",
	...
	// testing Wraps
	"data:map{x={y=2}}.mapf(fun(i,v,v.count()))":         "map[x:{1}]",
	"data:map{x=data:map{y=2}}.mapf(fun(i,v,v.count()))": "map[x:{1}]",
	"data:map{x=a}.mapf(fun(i,v,v.item(0)))":             "map[x:hello]",
	"data:map{x=c}.mapf(fun(i,v,v.count()))":             "map[x:{1}]",
